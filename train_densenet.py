...
# model_name = 'ddsm_densenet0529_3cls.h5'
# base_model = load_model(model_name)
# base_model.summary()
# print("load model successfully")
# base_model.trainable = True
# # x = base_model.get_layer('dropout').output
# # print("get layer successfully")
# # predictions = Dense(3, activation='softmax', name='dense_end')(x)
# # newnet = Model(inputs=base_model.input, outputs=predictions)
# newnet = Model(inputs=base_model.input, outputs=base_model.output)
...
newnet = keras.Sequential([
    net,
    layers.Dense(1024, activation='relu'),  # , kernel_regularizer=regularizers.l2(0.1)
    layers.BatchNormalization(),
    layers.Dropout(rate=0.5),  # alleviate overfitting
